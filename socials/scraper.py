...
def scrape_latest_video(username):
    ...
    for video_url in video_urls:
        ...
        data[username]["video_author_comment"] = video_soup.find(
            "span", {"class": "tiktok-j2a19r-SpanText efbd9f0"}
        ).text.strip()
        ...
        data["user_profile_pic"] = soup.find(
            "img", {"class": "tiktok-1zpj2q-ImgAvatar e1e9er4e1"}
        ).get("src")
        data["verified_status"] = (
            False if soup.find("circle", {"fill": "#20D5EC"}) == None else True
        )
        res = requests.get(
            f"https://api.douyin.wtf/tiktok_video_data/?tiktok_video_url={video_url}"
        )
        try:
