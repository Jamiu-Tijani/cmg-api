...
class InstagramService:
    ...
    def get_stories_feeds(self, **kwargs):
        username = list(kwargs.get("username").split(","))
        try:
            data = {}
            for user in username:
                user_recent_post = self.get_recent_post(username=user)["data"][user]
                data.update({user: {"feeds":user_recent_post}})
            for user in username:
                user_stories = self.get_stories(username=user)["data"][user]
                data[user]["stories"] = user_stories
            return dict(success="User stories and feeds fetch successful", status=200, data=data)
