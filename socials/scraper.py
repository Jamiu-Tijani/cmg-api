...
def scrape_latest_video(username):
    ...
    for data in video_soup.find_all("strong",{"class":"tiktok-cpjh4r-StrongText edu4zum2"}):
        user_latest_feed[username][data.get("data-e2e")] = data.text
    return dict(message="User latest feed fetch successful",status=200,data=user_latest_feed)
